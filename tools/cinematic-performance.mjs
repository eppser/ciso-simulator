...
const out='artifacts/v9/performance';fs.mkdirSync(out,{recursive:true});
...
try{
 ...
 await page.setViewport({width:1844,height:1214});await page.goto('http://127.0.0.1:5178/',{waitUntil:'networkidle0'});await page.click('[data-action=start][data-id=midcap]');
 ...
 assert.deepEqual(errors,[]);const report={at:new Date().toISOString(),errors,metrics,notes:'Native GPU. Same 1844×1214 CSS viewport as the supplied screenshot. First two samples render paused fixtures; third runs real simulation, movement, targeting, effects, shadows and reflections. No universal-device guarantee.'};
