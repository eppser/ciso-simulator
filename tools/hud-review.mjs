...
try{
 ...
 await p.waitForSelector('[data-action="operation-action"][data-id="infection-ad"]');await p.click('[data-action="operation-action"][data-id="infection-ad"]');await p.screenshot({path:`${out}/incident.png`});
 await p.click('[data-action="operation-action"][data-id="fbi-evidence"]');assert.ok(await p.$('[data-action="evidence-prepare"]'));await p.click('[data-action="evidence-prepare"]');assert.equal(await p.evaluate(()=>__app.game.evidence.state),'working');
 await p.click('[data-action="work-toggle"]');assert.match(await p.$eval('#work-tray',e=>e.textContent),/Preserve & review/);await p.click('[data-action="work-close"]');
 ...
 assert.deepEqual(errors,[]);fs.writeFileSync(`${out}/browser.json`,JSON.stringify({errors,mobile:r,checks:'menu identity retained, restart cancellation/confirmation, live menu, absent Governance button, priority cards, evidence action, top-bar work ledger, history, mobile'},null,2));console.log('HUD and menu checks passed');
