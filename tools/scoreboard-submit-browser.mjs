...
try{
 ...
 await p.click('[data-action=leaderboard]');await p.type('input[name=name]','QA Browser Submit');await p.type('input[name=superskill]','Temporary UI verification');await p.click('input[name=consent]');await p.click('.lb-submit button');
 ...
 await p.waitForFunction(()=>document.querySelector('.lb-rows')?.textContent.includes('QA Browser Submit'));await p.click('[data-view=history]');await p.waitForFunction(()=>document.querySelector('.lb-rows')?.textContent.includes('QA Browser Submit'));await p.screenshot({path:'artifacts/release/verified-ui-submit.png'});
 console.log('Real browser consent + form submission + published score + run-history visibility passed.');
}finally{
