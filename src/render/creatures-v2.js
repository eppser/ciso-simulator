...
export class AttackerLayer {
 ...
 sync(game,time,cameraQuat){const counts={virus:0,beetle:0,worm:0,boss:0,aithreat:0};let n=0,bars=0;
  for(const a of game.attackers){if(!a.alive||n>=MAX)continue;
   ...
   for(const {mesh,part} of this.pools[type]){
    const leg=/leg(\d)([LR])/.exec(part),segment=/segment(\d)/.exec(part);dummy.position.set(0,0,0);dummy.rotation.set(0,0,0);dummy.scale.setScalar(1);
    ...
    if(part.startsWith('orbit')){dummy.rotation.y=clock*(part.includes('0')?.11:-.08);}
    if(part.startsWith('nucleus'))dummy.scale.setScalar(1+Math.sin(clock*.6)*.06);
  ...
  for(const name in this.pools)for(const {mesh}of this.pools[name]){mesh.count=counts[name];mesh.instanceMatrix.needsUpdate=true;}
  this.rings.count=n;this.rings.instanceMatrix.needsUpdate=true;this.bars.count=bars;this.bars.instanceMatrix.needsUpdate=true;
 }
