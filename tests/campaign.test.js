...
describe('production campaign economy and people',()=>{
 ...
 it('scaled controls deduct exactly the quoted price, including flat firewall costs',()=>{const g=mk();g.budget=1000;for(const [type,x,y]of [['ips',7,3],['wall',13,10],['wall',13,11]]){const cost=g.buildCost(type),before=g.budget;expect(g.place(type,x,y).ok).toBe(true);expect(before-g.budget).toBe(cost);}});
 ...
 it('engineer capacity is enforced and purchased shifts add capacity',()=>{const g=mk();g.budget=1000;expect(g.buy('outside').ok).toBe(true);expect(g.buy('drill').ok).toBe(true);expect(g.buy('briefing').ok).toBe(false);expect(g.buy('soc1').ok).toBe(true);expect(g.buy('briefing').ok).toBe(true);expect(g.jobs).toHaveLength(3);});
